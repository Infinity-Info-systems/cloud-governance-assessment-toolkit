# Content Index

## Purpose

This index organizes the repo into a front door for cloud governance assessment, maturity scoring, and remediation planning.
Use it to get from scope definition to the exact assessment artifact you need.

## Entry Points

- [Assessment Plan](assessment-plan.md)
- [Toolkit Overview](./toolkit-overview.md)
- [Assessment Methodology](./assessment-methodology.md)
- [Scoring Model](./scoring-model.md)
- [Executive Reporting](./executive-reporting.md)

## Checklists and Questionnaires

- [Cloud Governance Checklist](../checklists/cloud-governance-checklist.md)
- [SRE Readiness Checklist](../checklists/sre-readiness-checklist.md)
- [DR Readiness Checklist](../checklists/dr-readiness-checklist.md)
- [FinOps Readiness Checklist](../checklists/finops-readiness-checklist.md)
- [Security and Compliance Checklist](../checklists/security-compliance-checklist.md)
- [Cloud Governance Questionnaire](../questionnaires/cloud-governance-questionnaire.md)
- [SRE and Observability Questionnaire](../questionnaires/sre-observability-questionnaire.md)
- [DR Questionnaire](../questionnaires/dr-questionnaire.md)
- [FinOps Questionnaire](../questionnaires/finops-questionnaire.md)

## Maturity, Scorecards, and Dashboards

- [Governance Maturity Model](../maturity-model/governance-maturity-model.md)
- [Cloud Maturity Scorecard](../scorecards/cloud-maturity-scorecard.md)
- [Governance Risk Scorecard](../scorecards/governance-risk-scorecard.md)
- [Reliability Scorecard](../scorecards/reliability-scorecard.md)
- [Assessment Dashboard](../dashboards/assessment-dashboard.md)

## Templates

- [Assessment Report Template](../templates/assessment-report-template.md)
- [Executive Summary Template](../templates/executive-summary-template.md)
- [Remediation Roadmap Template](../templates/remediation-roadmap-template.md)

## References

- [Bibliography](../references/bibliography.md)
- [Use-Case Notes](../evidence/use-case-notes.md)

## Reading Order

1. Toolkit overview
2. Assessment methodology
3. Scoring model
4. Relevant checklist and questionnaire
5. Scorecard and dashboard
6. Executive reporting and roadmap

## Shared Direction

Use the same section structure as the other core repos so assessment content stays reusable and easy to extend.
Keep each page short enough to review and clear enough to support evidence-based decisions.
